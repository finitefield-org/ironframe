...
This document describes the initial design for the `ironframe::scanner` module. The scanner extracts Tailwind-style class candidates from project files for JIT generation.
...
In the future, validation should call `ironframe::core::parse` and keep only tokens that parse successfully.
